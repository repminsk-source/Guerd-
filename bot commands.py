...
class AegisCommands(commands.Cog):
    ...
    async def help_command(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name=f"🎮 Мини-игры ({config.CURRENCY_NAME})",
            value=(
                "`/balance` `/daily` `/leaderboard`\n"
                "`/coinflip` — орёл/решка\n"
                "`/slots` — слот-машина\n"
                "`/dice` — дуэль на костях с другим участником"
            ),
            inline=False,
        )
        embed.set_footer(text="Команды настроек и модерации доступны только с правом «Управление сервером» / модератора.")
        await interaction.response.send_message(embed=embed, ephemeral=True)
